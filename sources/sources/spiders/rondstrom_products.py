...
class RondstromProductsSpider(scrapy.Spider):
    name = 'rondstrom_products'
    categories_urls = ["https://www.nordstrom.com/api/browse/browse/women/clothing?offset=0"]
    ...
    def get_headers_values(self):
        header_file = open('headers.csv', 'r')
        header_values = header_file.readlines()
        self.f_value = header_values[0].split(' = ')[1].replace('"','')
        self.b_value = header_values[1].split(' = ')[1].replace('"','')
        print (self.b_value)
        self.c_value = header_values[2].split(' = ')[1].replace('"','')
        self.d_value = header_values[3].split(' = ')[1].replace('"','')
        self.a_value = header_values[4].split(' = ')[1].replace('"','')
    ...
    def start_requests(self):
        for url in self.categories_urls:
            ...
            yield scrapy.Request(url, callback = self.parse_category,headers=headers)
